...
def create_content():
    ...
    if content_type in ['post', 'reel']:
        ...
        for f in followers:
            notif = Notification(user_id=f.follower_id, type='new_post', from_user_id=user.id, content_id=post.id, message=f'{user.username} posted anew {content_type}')
...
def hide_post(post_id):
    ...
    if not user:
        return jsonify({'error': 'Unauthorized'}), 401
    post = Post.query.get_or_404(post_id)
    hidden = HiddenPost(user_id=user.id, post_id=post_id)
...
def block_user(user_id):
    ...
    blocked = BlockedUser(blocker_id=user.id, blocked_id=user_id)
...
def update_profile():
    ...
    if data.get('dob'):
        user.dob = datetime.datetime.strptime(data['dob'], '%Y-%m-%d').date()
